...
GeneralFFT_1D::GeneralFFT_1D(void)
{
	ip = NULL;
	w = NULL;
	//wtable = NULL;
...
GeneralFFT_1D::GeneralFFT_1D(int n_i) {
	ip = NULL;
	w = NULL;
	/*wtable = NULL;*/
	//wtable_r = NULL;
	...
	//nac[0] = 0;
	 ...
	 //TWO_PI = 6.28318530717958623199592693708837032;
	
	n = n_i;


	//plan = SPLIT_RADIX;
	ip = new int[2 + (int) ceil(float(2 + (1 << (int) (log(n + 0.5) / log(2.0)) / 2)))];
	memset(ip,0,sizeof(int)*(2 + (int) ceil(float(2 + (1 << (int) (log(n + 0.5) / log(2.0)) / 2)))));
	...
	//ch = new float[n];
	//memset(ch,0,sizeof(float)*(n));
	//ch2 = new float[n*2];
	//memset(ch2,0,sizeof(float)*(n*2));
}
...
/**
	 ...
	 *            data to transform
	 */
void GeneralFFT_1D::complexForward(float* a) {
		complexForward(a, 0);
	}
	 ...
	 */
void GeneralFFT_1D::complexForward(float *a, int offa) {
		if (n == 1)
			return;
	 ...
	 */
void GeneralFFT_1D::complexInverse(float *a, bool scale) {
	 ...
	 *            if true then scaling is performed
	 */
void GeneralFFT_1D::complexInverse(float* a, int offa, bool scale) {
		if (n == 1)
			return;
	 ...
	 *            data to transform
	 */
 void GeneralFFT_1D::realForward(float* a) {
		realForward(a, 0);
	}
	 ...
	 *            index of the first element in array <code>a</code>
	 */
void GeneralFFT_1D::realForward(float* a, int offa) {
		if (n == 1)
			return;
	 ...
	 *            data to transform
	 */
void GeneralFFT_1D::realForwardFull(float* a) {
		realForwardFull(a, 0);
	}
	 ...
	 *            index of the first element in array <code>a</code>
	 */
void GeneralFFT_1D::realForwardFull(float* a, int offa) {
...
void GeneralFFT_1D::makewt(int nw) {
		int j, nwh, nw0, nw1;
...
void GeneralFFT_1D::makeipt(int nw) {
		int j, l, m, m2, p, q;
...
void GeneralFFT_1D::makect(int nc, float* c, int startc) {
		int j, nch;
		float delta, deltaj;
...
void GeneralFFT_1D::cftfsub(int n, float* a, int offa, int* ip, int nw, float* w) {
		if (n > 8) {
			if (n > 32) {
...
void GeneralFFT_1D::cftbsub(int n, float* a, int offa, int* ip, int nw, float* w) {
		if (n > 8) {
			if (n > 32) {
...
void GeneralFFT_1D::bitrv2(int n, int* ip, float* a, int offa) {
		int j1, k1, l, m, nh, nm;
		float xr, xi, yr, yi;
		int idx0, idx1, idx2;
...
void GeneralFFT_1D::bitrv2conj(int n, int* ip, float* a, int offa) {
		int j1, k1, l, m, nh, nm;
		float xr, xi, yr, yi;
		int idx0, idx1, idx2;
...
void GeneralFFT_1D::bitrv216(float* a, int offa) {
 ...
 void GeneralFFT_1D::bitrv216neg(float* a, int offa) {
...
void GeneralFFT_1D::bitrv208(float* a, int offa) {
...
void GeneralFFT_1D::bitrv208neg(float* a, int offa) {
...
void GeneralFFT_1D::cftf1st(int n, float* a, int offa, float* w, int startw) {
...
void GeneralFFT_1D::cftb1st(int n, float* a, int offa, float* w, int startw) {
		int j0, j1, j2, j3, k, m, mh;
		float wn4r, csc1, csc3, wk1r, wk1i, wk3r, wk3i, wd1r, wd1i, wd3r, wd3i;
		float x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i, y0r, y0i, y1r, y1i, y2r, y2i, y3r, y3i;
		int idx0, idx1, idx2, idx3, idx4, idx5;
...
void GeneralFFT_1D::cftrec4(int n, float* a, int offa, int nw, float* w) {
		int isplt, j, k, m;
...
int GeneralFFT_1D::cfttree(int n, int j, int k, float* a, int offa, int nw, float* w) {
		int i, isplt, m;
		int idx1 = offa - n;
		if ((k & 3) != 0) {
			isplt = k & 1;
...
void GeneralFFT_1D::cftleaf(int n, int isplt, float* a, int offa, int nw, float* w) {
		if (n == 512) {
 ...
 void GeneralFFT_1D::cftmdl1(int n, float* a, int offa, float* w, int startw) {
		int j0, j1, j2, j3, k, m, mh;
...
void GeneralFFT_1D::cftmdl2(int n, float* a, int offa, float* w, int startw) {
...
void GeneralFFT_1D::cftfx41(int n, float* a, int offa, int nw, float* w) {
		if (n == 128) {
			cftf161(a, offa, w, nw - 8);
			cftf162(a, offa + 32, w, nw - 32);
			cftf161(a, offa + 64, w, nw - 8);
...
void GeneralFFT_1D::cftf161(float* a, int offa, float* w, int startw) {
...
void GeneralFFT_1D::cftf162(float* a, int offa, float* w, int startw) {
...
void GeneralFFT_1D::cftf081(float* a, int offa, float* w, int startw) {
...
void GeneralFFT_1D::cftf082(float* a, int offa, float* w, int startw) {
...
void GeneralFFT_1D::cftf040(float* a, int offa) {
		float x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i;
...
void GeneralFFT_1D::cftb040(float* a, int offa) {
		float x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i;
...
void GeneralFFT_1D::cftx020(float* a, int offa) {
		float x0r, x0i;
		x0r = a[offa] - a[offa + 2];
...
void GeneralFFT_1D::cftxb020(float* a, int offa) {
		float x0r, x0i;
...
void GeneralFFT_1D::rftfsub(int n, float* a, int offa, int nc, float* c, int startc) {
		int k, kk, ks, m;
...
void GeneralFFT_1D::scale_fun(float m, float* a, int offa, bool complex) {
		float norm = (1.0 / m);
		int n2;
		if (complex) {
			n2 = 2 * n;
